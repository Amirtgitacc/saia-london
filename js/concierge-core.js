...
function systemPrompt(hire) {
  ...
  return [
    ...
    '- Delivery is symmetric, and there are only two options: our courier does BOTH journeys (delivery plus same-day collection), or the guest does both by collecting from and returning to NW3, which is free. There is no mixed option where we deliver and they return the mats.',
    '- The courier price is banded by postcode and covers both journeys: ' + KB.hire.currency + KB.delivery.bands.bandA + ' for our nearest areas, ' + KB.hire.currency + KB.delivery.bands.bandB + ' across central and inner London. Outer London and outside London are QUOTED per postcode → emit set_postcode and let the app say "by quote"; never guess a courier price for them.',
    '',
    KB.factSheet,
    '',
